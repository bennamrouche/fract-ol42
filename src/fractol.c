...
/*   Updated: 2023/01/01 20:15:44 by ebennamr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int ac, char **av)
{
	...
	if (ac != 2 && ac != 4)
		ft_error(ER_INV);
	if (ft_strncmp(av[1], MANDELBORT, ft_strlen(MANDELBORT)) == 0 && ac == 2)
		data_init(&data, 1, MANDELBORT, newcomplex(0, 0));
	else if (ft_strncmp(av[1], JULIA, ft_strlen(JULIA)) == 0
		&& jluiacheck(ac, av, &data))
		data_init(&data, 2, JULIA, newcomplex(data.c.x, data.c.y));
	else if (ft_strncmp(av[1], BURNING, ft_strlen(BURNING)) == 0 && ac == 2)
